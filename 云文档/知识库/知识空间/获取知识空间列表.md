...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看知识空间列表 <br> 查看、编辑和管理知识库 <br> 查看知识库 |
| 权限要求 | wiki:space:retrieve <br> wiki:wiki <br> wiki:wiki:readonly |
...
(async () => {
    for await (const item of await client.wiki.v2.space.listWithIterator({
            params: {
                        page_size:10,
                        lang:'zh',
            },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/wiki/v2/spaces?lang=zh&page_size=10&page_token=1565676577122621");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/wiki/v2/spaces?lang=zh&page_size=10&page_token=1565676577122621', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/wiki/v2/spaces?lang=zh&page_size=10&page_token=1565676577122621' \
