...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看、编辑和管理知识库 <br> 查看知识库 <br> 查看知识空间成员列表 |
| 权限要求 | wiki:member:retrieve <br> wiki:wiki <br> wiki:wiki:readonly |
...
def main():
    ...
    request: ListSpaceMemberRequest = ListSpaceMemberRequest.builder() \
        ...
        .page_size(1) \
        .page_token("NzA0NDg5NjIzMDA3MzE3MTk3Ml83MzYzMjE4NDkzNDI1NzI5NTM4") \
        .build()
    ...
    if not response.success():
        lark.logger.error(
            f"client.wiki.v2.space_member.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class ListSpaceMemberSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      ListSpaceMemberReq req = ListSpaceMemberReq.newBuilder()
             ...
             .pageSize(1)
             .pageToken("NzA0NDg5NjIzMDA3MzE3MTk3Ml83MzYzMjE4NDkzNDI1NzI5NTM4")
             .build();
...
client.wiki.v2.spaceMember.list({
        path: {
                space_id:'7375263209671884820',
        },
        params: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/wiki/v2/spaces/7375263209671884820/members?page_size=1&page_token=NzA0NDg5NjIzMDA3MzE3MTk3Ml83MzYzMjE4NDkzNDI1NzI5NTM4");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/wiki/v2/spaces/7375263209671884820/members?page_size=1&page_token=NzA0NDg5NjIzMDA3MzE3MTk3Ml83MzYzMjE4NDkzNDI1NzI5NTM4', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/wiki/v2/spaces/7375263209671884820/members?page_size=1&page_token=NzA0NDg5NjIzMDA3MzE3MTk3Ml83MzYzMjE4NDkzNDI1NzI5NTM4' \
...
func main() {
	...
	// 创建请求对象
	req := larkwiki.NewListSpaceMemberReqBuilder().
		SpaceId(`7375263209671884820`).
		PageSize(1).
		PageToken(`NzA0NDg5NjIzMDA3MzE3MTk3Ml83MzYzMjE4NDkzNDI1NzI5NTM4`).
		Build()
	...
	// 发起请求
	resp, err := client.Wiki.V2.SpaceMember.List(context.Background(), req)
