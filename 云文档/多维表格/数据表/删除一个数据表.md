...
删除一个数据表，如果多维表格中只剩最后一张数据表，则不允许被删除。
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/tables/tblsRc9GRRXKqhvW");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/tables/tblsRc9GRRXKqhvW', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/tables/tblsRc9GRRXKqhvW' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.bitable.v1.app_table.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class DeleteAppTableSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 发起请求
      DeleteAppTableResp resp = client.bitable().v1().appTable().delete(req);
...
client.bitable.v1.appTable.delete({
        path: {
                app_token:'appbcbWCzen6D8dezhoCH2RpMAh',
                table_id:'tblsRc9GRRXKqhvW',
        },
