...
删除多个数据表。如果多维表格中只剩最后一张数据表，则不允许被删除。
...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看、评论、编辑和管理多维表格 <br> 删除数据表 |
| 权限要求 | base:table:delete <br> bitable:app |
...
func main() {
	...
	req := larkbitable.NewBatchDeleteAppTableReqBuilder().
		...
		Body(larkbitable.NewBatchDeleteAppTableReqBodyBuilder().
			TableIds([]string{`tbl1TkhyTWDkSoZ3`}).
			Build()).
		Build()
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.bitable.v1.app_table.batch_delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
  ...
  public static void main(String arg[]) throws Exception {
      ...
      BatchDeleteAppTableReq req = BatchDeleteAppTableReq.newBuilder()
             ...
             .batchDeleteAppTableReqBody(BatchDeleteAppTableReqBody.newBuilder()
                 .tableIds(new String[]{"tbl1TkhyTWDkSoZ3"})
                  .build())
             .build();
...
client.bitable.v1.appTable.batchDelete({
        path: {
                app_token:'appbcbWCzen6D8dezhoCH2RpMAh',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/tables/batch_delete");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"table_ids\":[\"tbl1TkhyTWDkSoZ3\"]}";
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/tables/batch_delete', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/tables/batch_delete' \
