...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看、评论和导出多维表格 <br> 获取仪表盘信息 <br> 查看、评论、编辑和管理多维表格 |
| 权限要求 | base:dashboard:read <br> bitable:app <br> bitable:app:readonly |
...
client.bitable.v1.appDashboard.list({
        path: {
...
(async () => {
    for await (const item of await client.bitable.v1.appDashboard.listWithIterator({
            path: {
                    app_token:'bascng7vrxcxpig7geggXiCtadY',
            },
            params: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/bitable/v1/apps/bascng7vrxcxpig7geggXiCtadY/dashboards?page_size=10&page_token=blknkqrP3RqUkcAW");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/bitable/v1/apps/bascng7vrxcxpig7geggXiCtadY/dashboards?page_size=10&page_token=blknkqrP3RqUkcAW', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/bitable/v1/apps/bascng7vrxcxpig7geggXiCtadY/dashboards?page_size=10&page_token=blknkqrP3RqUkcAW' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.bitable.v1.app_dashboard.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
