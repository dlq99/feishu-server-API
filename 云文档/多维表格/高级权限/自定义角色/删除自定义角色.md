...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看、评论、编辑和管理多维表格 <br> 删除自定义角色 |
| 权限要求 | base:role:delete <br> bitable:app |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.bitable.v1.app_role.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.bitable.v1.appRole.delete({
        path: {
                app_token:'appbcbWCzen6D8dezhoCH2RpMAh',
                role_id:'roljRpwIUt',
        },
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/roles/roljRpwIUt', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/roles/roljRpwIUt' \
