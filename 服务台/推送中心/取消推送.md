...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| is_recall | boolean | 是 | 是否召回已发送的消息,新人入职消息同样适用 <br> **示例**: true  |
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6981801914270744596/cancel_send");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"is_recall\":true}";
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6981801914270744596/cancel_send', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6981801914270744596/cancel_send' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Helpdesk.V1.Notification.CancelSend(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.helpdesk.v1.notification.cancel_send failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.helpdesk.v1.notification.cancelSend({
        path: {
                notification_id:'6981801914270744596',
        },
        data: {
