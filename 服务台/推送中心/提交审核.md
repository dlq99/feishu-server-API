...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| reason | string | 是 | 提交审批理由 <br> **示例**: 测试发送消息  |
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6985032626234982420/submit_approve");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"reason\":\"测试发送消息\"}";
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6985032626234982420/submit_approve', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6985032626234982420/submit_approve' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Helpdesk.V1.Notification.SubmitApprove(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.helpdesk.v1.notification.submit_approve failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.helpdesk.v1.notification.submitApprove({
        path: {
                notification_id:'6985032626234982420',
        },
        data: {
