...
client.helpdesk.v1.notification.cancelApprove({
        path: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6981801914270744596/cancel_approve");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6981801914270744596/cancel_approve', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/helpdesk/v1/notifications/6981801914270744596/cancel_approve' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.helpdesk.v1.notification.cancel_approve failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
