...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| name | string | 否 | 名称 <br> **示例**: 创建团队和邀请成员  |
| parent_id | string | 否 | 父知识库分类ID <br> **示例**: 0  |
...
public class PatchCategorySample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 发起请求
      PatchCategoryResp resp = client.helpdesk().v1().category().patch(req);
...
client.helpdesk.v1.category.patch({
        path: {
                id:'6948728206392295444',
        },
        data: {
...
var body = "{\"name\":\"创建团队和邀请成员\",\"parent_id\":\"0\"}";
...
$request = new Request('PATCH', 'https://open.feishu.cn/open-apis/helpdesk/v1/categories/6948728206392295444', $headers, $body);
...
```bash
curl -i -X PATCH 'https://open.feishu.cn/open-apis/helpdesk/v1/categories/6948728206392295444' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Helpdesk.V1.Category.Patch(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.helpdesk.v1.category.patch failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
