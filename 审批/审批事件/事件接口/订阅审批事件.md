...
当应用[订阅审批事件](/ssl:ttdoc/ukTMukTMukTM/uUTNz4SN1MjL1UzM)后，需要调用该接口指定审批定义 Code（approval_code）开启订阅，开启后应用才可以接收该审批定义对应的事件。
...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看、创建、更新、删除审批应用相关信息 <br> 查看、创建、更新、删除原生审批定义相关信息 |
...
client.approval.v4.approval.subscribe({
        path: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/approval/v4/approvals/7C468A54-8745-2245-9675-08B7C63E7A85/subscribe");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/approval/v4/approvals/7C468A54-8745-2245-9675-08B7C63E7A85/subscribe', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/approval/v4/approvals/7C468A54-8745-2245-9675-08B7C63E7A85/subscribe' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.approval.v4.approval.subscribe failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
