...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| member_type | string | 是 | 用户组成员的类型，目前仅支持选择 user。 <br> **示例**: user <br> **可选值**:<br>- user: 用户类型。 <br> |
| member_id_type | string | 是 | 当 `member_type` 取值为 `user`时，通过该参数设置用户 ID 类型。 <br> **示例**: open_id <br> **可选值**:<br>- open_id: 标识一个用户在某个应用中的身份。同一个用户在不同应用中的 Open ID 不同。 <br>- union_id: 标识一个用户在某个应用开发商下的身份。同一用户在同一开发商下的应用中的 Union ID 是相同的，在不同开发商下的应用中的 Union ID 是不同的。通过 Union ID，应用开发商可以把同个用户在多个应用中的身份关联起来。 <br>- user_id: 标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内，一个用户的 User ID 在所有应用中都保持一致。User ID 主要用于在不同的应用间打通用户数据。 <br> |
| member_id | string | 是 | 添加的用户 ID，ID 类型与 member_id_type 的取值保持一致。不同类型的 ID 获取方式可参见：<br>- [如何获取用户 open_id](/ssl:ttdoc/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)<br>- [如何获取用户 union_id](/ssl:ttdoc/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-union-id)<br>- [如何获取用户 user_id](/ssl:ttdoc/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id) <br> **示例**: ou_7dab8a3d3cdcc9da365777c7ad535d62  |
...
client.contact.v3.groupMember.add({
        path: {
                group_id:'g281721',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/contact/v3/group/g281721/member/add");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"member_id\":\"ou_7dab8a3d3cdcc9da365777c7ad535d62\",\"member_id_type\":\"open_id\",\"member_type\":\"user\"}";
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/contact/v3/group/g281721/member/add', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/contact/v3/group/g281721/member/add' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Contact.V3.GroupMember.Add(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.contact.v3.group_member.add failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
