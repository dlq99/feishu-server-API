...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| members | undefined[] | 是 | 待添加为角色成员的用户 ID 列表，以 `["xxx", "yyy"]` 数组格式进行传值。ID 类型需要和查询参数 user_id_type 的取值保持一致。 <br> **示例**: ["qwer","qweq"] <br> **数据校验规则**:<br>最大长度: 100最小长度: 1 |
...
public class BatchCreateFunctionalRoleMemberSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 发起请求
      BatchCreateFunctionalRoleMemberResp resp = client.contact().v3().functionalRoleMember().batchCreate(req);
...
client.contact.v3.functionalRoleMember.batchCreate({
        path: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/contact/v3/functional_roles/7vrj3vk70xk7v5r/members/batch_create?user_id_type=open_id");
client.Timeout = -1;
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/contact/v3/functional_roles/7vrj3vk70xk7v5r/members/batch_create?user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/contact/v3/functional_roles/7vrj3vk70xk7v5r/members/batch_create?user_id_type=open_id' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.contact.v3.functional_role_member.batch_create failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
