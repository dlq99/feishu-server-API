...
$body = '// 无请求体参数，需传入请求头与路径参数。';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/contact/v3/unit/BU121', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/contact/v3/unit/BU121' \
-H 'Authorization: Bearer t-7f1b******8e560' \
-H 'Content-Type: application/json' \
-d '// 无请求体参数，需传入请求头与路径参数。'
```
...
func main() {
	...
	req := larkcontact.NewDeleteUnitReqBuilder().
		UnitId(`BU121`).
		Body(larkcontact.NewDeleteUnitReqBodyBuilder().
			Build()).
		Build()

	// 发起请求
	resp, err := client.Contact.V3.Unit.Delete(context.Background(), req)
...
def main():
    ...
    request: DeleteUnitRequest = DeleteUnitRequest.builder() \
        .unit_id("BU121") \
        .request_body(DeleteUnitRequestBody.builder()
            .build()) \
        .build()
    ...
    if not response.success():
        lark.logger.error(
            f"client.contact.v3.unit.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class DeleteUnitSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      DeleteUnitReq req = DeleteUnitReq.newBuilder()
             .unitId("BU121")
             .deleteUnitReqBody(DeleteUnitReqBody.newBuilder()
                  .build())
             .build();

      // 发起请求
      DeleteUnitResp resp = client.contact().v3().unit().delete(req);
