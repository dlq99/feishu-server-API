...
| 400 | 41206 | functional role has members, can not delete | 角色内含有成员，不可删除。你需要先[删除角色下的成员](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/functional_role-member/batch_delete)，然后重试。 |
...
client.contact.v3.functionalRole.delete({
        path: {
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/contact/v3/functional_roles/7vrj3vk70xk7v5r', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/contact/v3/functional_roles/7vrj3vk70xk7v5r' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Contact.V3.FunctionalRole.Delete(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.contact.v3.functional_role.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
