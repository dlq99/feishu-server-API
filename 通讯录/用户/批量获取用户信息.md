...
| 支持的应用类型 | custom  isv |
| 权限要求 | 获取用户受雇信息 <br> 获取成员所在部门路径 <br> 读取通讯录 <br> 获取通讯录基本信息 <br> 查询用户席位信息 <br> 分配用户席位 <br> 获取用户组织架构信息 <br> 获取用户 user ID <br> 获取用户性别 <br> 获取用户手机号 <br> 查看成员工号 <br> 查询用户所属的工作序列 <br> 查询用户职级 <br> 以应用身份读取通讯录 <br> 查看成员的虚线上级 ID <br> 获取用户基本信息 <br> 以应用身份访问通讯录 <br> 获取用户邮箱信息 <br> 查看成员数据驻留地 |
...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| user_ids | array | 是 | 用户ID。ID 类型与查询参数 `user_id_type` 保持一致。
