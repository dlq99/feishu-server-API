...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.contact.v3.job_family.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.contact.v3.jobFamily.list({
        params: {
                page_size:10,
                page_token:'3',
                name:'产品',
...
(async () => {
    for await (const item of await client.contact.v3.jobFamily.listWithIterator({
            params: {
                        page_size:10,
                        name:'产品',
            },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/contact/v3/job_families?name=%E4%BA%A7%E5%93%81&page_size=10&page_token=3");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/contact/v3/job_families?name=%E4%BA%A7%E5%93%81&page_size=10&page_token=3', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/contact/v3/job_families?name=%E4%BA%A7%E5%93%81&page_size=10&page_token=3' \
