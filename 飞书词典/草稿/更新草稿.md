...
```bash
curl -i -X PUT 'https://open.feishu.cn/open-apis/lingo/v1/drafts/7241543272228814852?user_id_type=open_id' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.lingo.v1.draft.update failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/lingo/v1/drafts/7241543272228814852?user_id_type=open_id");
client.Timeout = -1;
...
$request = new Request('PUT', 'https://open.feishu.cn/open-apis/lingo/v1/drafts/7241543272228814852?user_id_type=open_id', $headers, $body);
