...
| 支持的应用类型 | custom  isv |
| 权限要求 | 获取企业信息 <br> 查看企业的完整域名 |
| 权限要求 | tenant:tenant:readonly |
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/tenant/v2/tenant/query");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/tenant/v2/tenant/query', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/tenant/v2/tenant/query' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.tenant.v2.tenant.query failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
