...
| 403 | 1470403 | 缺少对任务的访问权限。 | 检查调用身份是否有权限访问任务数据。详情见[任务功能概述](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。 |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.task.v2.task.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/task/v2/tasks/e297ddff-06ca-4166-b917-4ce57cd3a7a0?user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/task/v2/tasks/e297ddff-06ca-4166-b917-4ce57cd3a7a0?user_id_type=open_id' \
