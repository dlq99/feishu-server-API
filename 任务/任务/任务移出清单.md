...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| task_guid | string | 要从清单移除的任务的全局唯一ID |
...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| tasklist_guid | string | 是 | 要移除的清单的全局唯一ID <br> **示例**: d300a75f-c56a-4be9-80d1-e47653028ceb <br> **数据校验规则**:<br>最大长度: 100 |
...
| 403 | 1470403 | 缺少任务移出清单的权限。 | 检查调用身份是否有清单的可编辑权限。详情见[清单功能概述](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/task-v2/tasklist/overview)中的“清单是如何鉴权的？“章节。 |
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/task/v2/tasks/d300a75f-c56a-4be9-80d1-e47653028ceb/remove_tasklist?user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/task/v2/tasks/d300a75f-c56a-4be9-80d1-e47653028ceb/remove_tasklist?user_id_type=open_id' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.task.v2.task.remove_tasklist failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/task/v2/tasks/d300a75f-c56a-4be9-80d1-e47653028ceb/remove_tasklist?user_id_type=open_id");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"tasklist_guid\":\"d300a75f-c56a-4be9-80d1-e47653028ceb\"}";
