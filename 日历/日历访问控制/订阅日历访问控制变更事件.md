...
| 支持的应用类型 | custom  isv |
| 权限要求 | 更新日历及日程信息 <br> 读取日历访问控制权限 <br> 获取日历、日程及忙闲信息 |
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/acls/subscription' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Calendar.V4.CalendarAcl.Subscription(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.calendar.v4.calendar_acl.subscription failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.calendar.v4.calendarAcl.subscription({
        path: {
                calendar_id:'feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn',
        },
},
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/acls/subscription");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/acls/subscription', $headers, $body);
