...
| 支持的应用类型 | custom  isv |
| 权限要求 | 删除日历访问控制权限 <br> 更新日历及日程信息 |
| 权限要求 | calendar:calendar <br> calendar:calendar.acl:delete |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.calendar.v4.calendar_acl.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.calendar.v4.calendarAcl.delete({
        path: {
                calendar_id:'feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn',
                acl_id:'user_xxxxxx',
        },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/acls/user_xxxxxx");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/acls/user_xxxxxx', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/acls/user_xxxxxx' \
