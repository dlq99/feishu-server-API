...
| 支持的应用类型 | custom  isv |
| 权限要求 | 更新日历及日程信息 <br> 删除日历 |
| 权限要求 | calendar:calendar <br> calendar:calendar:delete |
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.calendar.v4.calendar.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
