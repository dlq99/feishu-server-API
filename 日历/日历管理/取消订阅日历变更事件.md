...
调用该接口为当前用户身份取消订阅[日历变更事件](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/events/changed)。
...
$body = 'null';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/calendar/v4/calendars/unsubscription', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/calendar/v4/calendars/unsubscription' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.calendar.v4.calendar.unsubscription failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.calendar.v4.calendar.unsubscription({
},
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/calendar/v4/calendars/unsubscription");
client.Timeout = -1;
