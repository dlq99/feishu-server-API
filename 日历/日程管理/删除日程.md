...
| 支持的应用类型 | custom  isv |
| 权限要求 | 更新日历及日程信息 <br> 删除日程 |
| 权限要求 | calendar:calendar <br> calendar:calendar.event:delete |
...
| 403 | 193003 | event is deleted | 日程已经被删除。你需要检查并改为正确的日程 ID。日程获取方式：<br><br>- [创建日程](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-event/create)会返回日程 ID<br>- [获取日程列表](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-event/list)<br>- [搜索日程](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-event/search) |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.calendar.v4.calendar_event.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.calendar.v4.calendarEvent.delete({
        path: {
                calendar_id:'feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn',
                event_id:'xxxxxxxxx_0',
        },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/events/xxxxxxxxx_0?need_notification=false");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/events/xxxxxxxxx_0?need_notification=false', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/calendar/v4/calendars/feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn/events/xxxxxxxxx_0?need_notification=false' \
