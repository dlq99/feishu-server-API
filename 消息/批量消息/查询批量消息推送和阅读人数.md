...
[批量发送消息](/ssl:ttdoc/ukTMukTMukTM/ucDO1EjL3gTNx4yN4UTM)后，可通过该接口查询消息推送的总人数以及消息已读人数。
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v1/batch_messages/bm_dc13264520392913993dd051dba21dcf/read_user");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/im/v1/batch_messages/bm_dc13264520392913993dd051dba21dcf/read_user', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/im/v1/batch_messages/bm_dc13264520392913993dd051dba21dcf/read_user' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v1.batch_message.read_user failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.im.v1.batchMessage.readUser({
        path: {
                batch_message_id:'bm_dc13264520392913993dd051dba21dcf',
        },
},
