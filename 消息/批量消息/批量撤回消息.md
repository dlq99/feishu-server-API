...
该接口用于撤回通过[批量发送消息](/ssl:ttdoc/ukTMukTMukTM/ucDO1EjL3gTNx4yN4UTM)接口发送的消息。
...
| 支持的应用类型 | custom  isv |
| 权限要求 | 给多个用户批量发消息 <br> 给一个或多个部门的成员批量发消息 |
...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| batch_message_id | string | 待撤回的批量消息任务 ID，该 ID 为[批量发送消息](/ssl:ttdoc/ukTMukTMukTM/ucDO1EjL3gTNx4yN4UTM)接口返回值中的`message_id`字段，用于标识一次批量发送消息请求。 |
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v1/batch_messages/bm-dc13264520392913993dd051dba21dcf");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/im/v1/batch_messages/bm-dc13264520392913993dd051dba21dcf', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/im/v1/batch_messages/bm-dc13264520392913993dd051dba21dcf' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Im.V1.BatchMessage.Delete(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v1.batch_message.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.im.v1.batchMessage.delete({
        path: {
                batch_message_id:'bm-dc13264520392913993dd051dba21dcf',
        },
},
