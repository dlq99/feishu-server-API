...
| 支持的应用类型 | custom  isv |
| 权限要求 | 获取与发送单聊、群组消息 <br> 添加、 取消 Pin 消息 <br> 以应用的身份发消息 |
...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| message_id | string | 待移除 Pin 的消息 ID。ID 获取方式：
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v1.pin.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class DeletePinSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 创建请求对象
      DeletePinReq req = DeletePinReq.newBuilder()
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v1/pins/om_dc13264520392913993dd051dba21dcf");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/im/v1/pins/om_dc13264520392913993dd051dba21dcf', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/im/v1/pins/om_dc13264520392913993dd051dba21dcf' \
