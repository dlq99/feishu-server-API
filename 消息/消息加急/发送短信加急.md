...
| 支持的应用类型 | custom  isv |
| 权限要求 | 发送短信加急消息 <br> 发送短信加急消息（历史版本） <br> 获取用户 user ID |
| 权限要求 | im:message.urgent:sms <br> im:message.urgent:sms_send |
...
client.im.v1.message.urgentSms({
        path: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v1/messages/om_dc13264520392913993dd051dba21dcf/urgent_sms?user_id_type=open_id");
client.Timeout = -1;
...
$request = new Request('PATCH', 'https://open.feishu.cn/open-apis/im/v1/messages/om_dc13264520392913993dd051dba21dcf/urgent_sms?user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X PATCH 'https://open.feishu.cn/open-apis/im/v1/messages/om_dc13264520392913993dd051dba21dcf/urgent_sms?user_id_type=open_id' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v1.message.urgent_sms failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
