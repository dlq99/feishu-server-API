...
client.attendance.v1.group.list({
        params: {
                page_size:10,
                page_token:'wgNOR1rmxogRvAsGl6CXlQ==',
        },
...
(async () => {
    for await (const item of await client.attendance.v1.group.listWithIterator({
            params: {
                        page_size:10,
            },
    },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/attendance/v1/groups?page_size=10&page_token=wgNOR1rmxogRvAsGl6CXlQ%3D%3D");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/attendance/v1/groups?page_size=10&page_token=wgNOR1rmxogRvAsGl6CXlQ%3D%3D', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/attendance/v1/groups?page_size=10&page_token=wgNOR1rmxogRvAsGl6CXlQ%3D%3D' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Attendance.V1.Group.List(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.attendance.v1.group.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
