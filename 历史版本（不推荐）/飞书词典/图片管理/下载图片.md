...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/baike/v1/files/boxbcEcmKiD3***vgqWTpvdc7jc/download' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.baike.v1.file.download failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/baike/v1/files/boxbcEcmKiD3***vgqWTpvdc7jc/download");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/baike/v1/files/boxbcEcmKiD3***vgqWTpvdc7jc/download', $headers, $body);
