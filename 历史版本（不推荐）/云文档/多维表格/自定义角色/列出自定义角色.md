...
| 支持的应用类型 | custom  isv |
| 权限要求 | 列出自定义角色 <br> 查看、评论、编辑和管理多维表格 <br> 查看、评论和导出多维表格 |
| 权限要求 | base:role:read <br> bitable:app <br> bitable:app:readonly |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.bitable.v1.app_role.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.bitable.v1.appRole.list({
        path: {
                app_token:'appbcbWCzen6D8dezhoCH2RpMAh',
        },
        params: {
...
(async () => {
    for await (const item of await client.bitable.v1.appRole.listWithIterator({
            path: {
                    app_token:'appbcbWCzen6D8dezhoCH2RpMAh',
            },
            params: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/roles?page_size=10&page_token=eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/roles?page_size=10&page_token=eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/bitable/v1/apps/appbcbWCzen6D8dezhoCH2RpMAh/roles?page_size=10&page_token=eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0' \
