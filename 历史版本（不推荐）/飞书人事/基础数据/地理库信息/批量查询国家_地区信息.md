...
client.corehr.v1.countryRegion.list({
        params: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v1/country_regions?page_size=100&page_token=1231231987");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/corehr/v1/country_regions?page_size=100&page_token=1231231987', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/corehr/v1/country_regions?page_size=100&page_token=1231231987' \
...
func main() {
	...
	// 创建请求对象
	req := larkcorehr.NewListCountryRegionReqBuilder().
		PageToken(`1231231987`).
		PageSize(`100`).
		Build()

	// 发起请求
	resp, err := client.Corehr.V1.CountryRegion.List(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v1.country_region.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
