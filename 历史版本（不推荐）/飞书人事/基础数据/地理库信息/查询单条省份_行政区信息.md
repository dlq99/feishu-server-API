...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/corehr/v1/subdivisions/67489937334909845' \
...
func main() {
	...
	// 创建请求对象
	req := larkcorehr.NewGetSubdivisionReqBuilder().
		SubdivisionId(`67489937334909845`).
		Build()

	// 发起请求
	resp, err := client.Corehr.V1.Subdivision.Get(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v1.subdivision.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
// 复制该 Demo 后, 需要将 "YOUR_APP_ID", "YOUR_APP_SECRET" 替换为自己应用的 APP_ID, APP_SECRET.
// 以下示例代码默认根据文档示例值填充，如果存在代码问题，请在 API 调试台填上相关必要参数后再复制代码使用
public class GetSubdivisionSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 创建请求对象
      GetSubdivisionReq req = GetSubdivisionReq.newBuilder()
             .subdivisionId("67489937334909845")
             .build();
...
client.corehr.v1.subdivision.get({
        path: {
                subdivision_id:'67489937334909845',
        },
},
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v1/subdivisions/67489937334909845");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/corehr/v1/subdivisions/67489937334909845', $headers, $body);
