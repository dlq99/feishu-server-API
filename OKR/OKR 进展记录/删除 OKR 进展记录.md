...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| progress_id | string | 待删除的 OKR进展记录 ID |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.okr.v1.progress_record.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class DeleteProgressRecordSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 创建请求对象
      DeleteProgressRecordReq req = DeleteProgressRecordReq.newBuilder()
             .progressId("7041857032248410131")
             .build();
...
client.okr.v1.progressRecord.delete({
        path: {
                progress_id:'7041857032248410131',
        },
},
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/okr/v1/progress_records/7041857032248410131");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/okr/v1/progress_records/7041857032248410131', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/okr/v1/progress_records/7041857032248410131' \
