...
public class ListPeriodRuleSample{
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/okr/v1/period_rules");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/okr/v1/period_rules', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/okr/v1/period_rules' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Okr.V1.PeriodRule.List(context.Background())
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.okr.v1.period_rule.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
