...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/corehr/v2/report_detail_row/batchDelete', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/corehr/v2/report_detail_row/batchDelete' \
...
func main() {
	...
	// 创建请求对象
	req := larkcorehr.NewBatchDeleteReportDetailRowReqBuilder().
	...
	// 发起请求
	resp, err := client.Corehr.V2.ReportDetailRow.BatchDelete(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v2.report_detail_row.batch_delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
// 复制该 Demo 后, 需要将 "YOUR_APP_ID", "YOUR_APP_SECRET" 替换为自己应用的 APP_ID, APP_SECRET.
// 以下示例代码默认根据文档示例值填充，如果存在代码问题，请在 API 调试台填上相关必要参数后再复制代码使用
public class BatchDeleteReportDetailRowSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 创建请求对象
      BatchDeleteReportDetailRowReq req = BatchDeleteReportDetailRowReq.newBuilder()
...
WorkforcePlanEaiDetail.newBuilder()
  ...
  .build()
      ...
      // 发起请求
      BatchDeleteReportDetailRowResp resp = client.corehr().v2().reportDetailRow().batchDelete(req);
...
client.corehr.v2.reportDetailRow.batchDelete({
        data: {
                workforce_plan_id:'7435976673130317356',
                centralized_reporting_project_id:'7436723164333753900',
                items:[
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v2/report_detail_row/batchDelete");
client.Timeout = -1;
