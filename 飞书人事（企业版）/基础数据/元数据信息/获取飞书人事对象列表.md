...
public class ListObjectApiNameCustomFieldSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 创建请求对象
      ListObjectApiNameCustomFieldReq req = ListObjectApiNameCustomFieldReq.newBuilder()
             .pageToken("11")
             .pageSize("100")
             .build();
...
client.corehr.v1.customField.listObjectApiName({
        params: {
                page_token:'11',
                page_size:'100',
        },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v1/custom_fields/list_object_api_name?page_size=100&page_token=11");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/corehr/v1/custom_fields/list_object_api_name?page_size=100&page_token=11', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/corehr/v1/custom_fields/list_object_api_name?page_size=100&page_token=11' \
...
func main() {
	...
	// 创建请求对象
	req := larkcorehr.NewListObjectApiNameCustomFieldReqBuilder().
		PageToken(`11`).
		PageSize(`100`).
		Build()

	// 发起请求
	resp, err := client.Corehr.V1.CustomField.ListObjectApiName(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v1.custom_field.list_object_api_name failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
