...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| bank_id_list | undefined[] | 否 | 银行 ID 列表，可通过[查询银行信息](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/corehr-v2/basic_info-bank/search)列举，或从[批量查询员工信息](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/corehr-v2/employee/batch_get)等接口返回的 `person_info.bank_account_list.bank_id_v2` 字段中获取 <br> **示例**:  <br> **数据校验规则**:<br>最大长度: 100 |
| bank_branch_id_list | undefined[] | 否 | 支行 ID 列表，可通过[批量查询员工信息](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/corehr-v2/employee/batch_get)等接口返回的 `person_info.bank_account_list.bank_branch_id_v2` 字段获取 <br> **示例**:  <br> **数据校验规则**:<br>最大长度: 100 |
| bank_branch_name_list | undefined[] | 否 | 支行名称列表，支持对支行名称精确搜索 <br> **示例**:  <br> **数据校验规则**:<br>最大长度: 100 |
| code_list | undefined[] | 否 | 金融分支机构编码（联行号）列表，支持对金融分支机构编码精确搜索 <br> **示例**:  <br> **数据校验规则**:<br>最大长度: 100 |
