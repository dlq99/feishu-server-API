...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| employee_type_id | string | 雇员类型 ID
...
| default_employee_type | boolean | 否 | 是否为默认人员类型，每个租户只能定义一个默认人员类型 <br> **示例**: true  |
...
client.corehr.v1.employeeType.patch({
        path: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v1/employee_types/6969828847931885087?client_token=12454646");
client.Timeout = -1;
...
$request = new Request('PATCH', 'https://open.feishu.cn/open-apis/corehr/v1/employee_types/6969828847931885087?client_token=12454646', $headers, $body);
...
```bash
curl -i -X PATCH 'https://open.feishu.cn/open-apis/corehr/v1/employee_types/6969828847931885087?client_token=12454646' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v1.employee_type.patch failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
