...
| 支持的应用类型 | custom  isv |
| 权限要求 | 更新核心人事信息 <br> 更新预置数据信息 <br> 获取预置数据信息 <br> 获取核心人事信息 |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v1.working_hours_type.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.corehr.v1.workingHoursType.list({
        params: {
                page_token:'1231231987',
                page_size:'100',
        },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v1/working_hours_types?page_size=100&page_token=1231231987");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/corehr/v1/working_hours_types?page_size=100&page_token=1231231987', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/corehr/v1/working_hours_types?page_size=100&page_token=1231231987' \
