...
| 支持的应用类型 | custom  isv |
| 权限要求 | 更新地点信息 <br> 获取核心人事信息 <br> 获取地点信息 <br> 更新核心人事信息 |
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/corehr/v1/locations?page_size=100&page_token=1231231987' \
...
func main() {
	...
	// 创建请求对象
	req := larkcorehr.NewListLocationReqBuilder().
		PageToken(`1231231987`).
		PageSize(`100`).
		Build()

	// 发起请求
	resp, err := client.Corehr.V1.Location.List(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.corehr.v1.location.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class ListLocationSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 发起请求
      ListLocationResp resp = client.corehr().v1().location().list(req);
...
client.corehr.v1.location.list({
        params: {
                page_token:'1231231987',
                page_size:'100',
        },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/corehr/v1/locations?page_size=100&page_token=1231231987");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/corehr/v1/locations?page_size=100&page_token=1231231987', $headers, $body);
