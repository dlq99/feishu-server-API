...
| 支持的应用类型 | custom |
| 权限要求 | 获取用户登录信息（mask） <br> 获取用户 user ID |
| 权限要求 | passport:session_mask:readonly |
...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| user_ids | undefined[] | 否 | 用户 ID <br> **示例**: ["47f621ff"] <br> **数据校验规则**:<br>最大长度: 100 |
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/passport/v1/sessions/query?user_id_type=open_id' \
...
func main() {
	...
	req := larkpassport.NewQuerySessionReqBuilder().
		...
		Body(larkpassport.NewQuerySessionReqBodyBuilder().
			UserIds([]string{`7aeddb6a1`, `7aeddb6a2`, `7aeddb6a3`}).
			Build()).
		Build()
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.passport.v1.session.query failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
  ...
  public static void main(String arg[]) throws Exception {
      ...
      QuerySessionReq req = QuerySessionReq.newBuilder()
             ...
             .querySessionReqBody(QuerySessionReqBody.newBuilder()
                 .userIds(new String[]{"7aeddb6a1","7aeddb6a2","7aeddb6a3"})
                  .build())
             .build();
...
client.passport.v1.session.query({
        params: {
                user_id_type:'open_id',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/passport/v1/sessions/query?user_id_type=open_id");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"user_ids\":[\"7aeddb6a1\",\"7aeddb6a2\",\"7aeddb6a3\"]}";
...
$body = '{"user_ids":["7aeddb6a1","7aeddb6a2","7aeddb6a3"]}';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/passport/v1/sessions/query?user_id_type=open_id', $headers, $body);
