...
该 API 用于获取某个飞书智能伙伴应用的运行（Run）的详细信息；包括运行的状态、结束时间等。
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/aily/v1/sessions/session_4dfunz7sp1g8m/runs/run_4dfrxvctjqzzj");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/aily/v1/sessions/session_4dfunz7sp1g8m/runs/run_4dfrxvctjqzzj', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/aily/v1/sessions/session_4dfunz7sp1g8m/runs/run_4dfrxvctjqzzj' \
...
func main() {
	...
	// 创建请求对象
	req := larkaily.NewGetAilySessionRunReqBuilder().
		AilySessionId(`session_4dfunz7sp1g8m`).
		RunId(`run_4dfrxvctjqzzj`).
		Build()

	// 发起请求
	resp, err := client.Aily.V1.AilySessionRun.Get(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.aily.v1.aily_session_run.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.aily.v1.ailySessionRun.get({
        path: {
                aily_session_id:'session_4dfunz7sp1g8m',
                run_id:'run_4dfrxvctjqzzj',
        },
