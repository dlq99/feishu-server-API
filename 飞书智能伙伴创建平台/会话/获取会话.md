...
该 API 用于获取与某个飞书智能伙伴应用的一次会话（Session）的详细信息，包括会话的状态、渠道上下文、创建时间等。
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.aily.v1.aily_session.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.aily.v1.ailySession.get({
        path: {
                aily_session_id:'session_4dfunz7sp1g8m',
        },
},
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/aily/v1/sessions/session_4dfunz7sp1g8m', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/aily/v1/sessions/session_4dfunz7sp1g8m' \
...
func main() {
	...
	// 创建请求对象
	req := larkaily.NewGetAilySessionReqBuilder().
		AilySessionId(`session_4dfunz7sp1g8m`).
		Build()

	// 发起请求
	resp, err := client.Aily.V1.AilySession.Get(context.Background(), req)
