...
| 支持的应用类型 | custom  isv |
| 权限要求 | 查看智能门禁设备列表 <br> 查看、更新智能门禁用户 <br> 获取用户 user ID |
| 权限要求 | acs:devices:readonly <br> acs:users |
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/acs/v1/rule_external?device_id=7296700518380863767&user_id_type=open_id");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/acs/v1/rule_external?device_id=7296700518380863767&user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/acs/v1/rule_external?device_id=7296700518380863767&user_id_type=open_id' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.acs.v1.rule_external.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.acs.v1.ruleExternal.get({
        params: {
                device_id:'7296700518380863767',
                user_id_type:'open_id',
        },
