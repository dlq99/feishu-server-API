...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.application.v6.scope.apply failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.application.v6.scope.apply({
},
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/application/v6/scopes/apply");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/application/v6/scopes/apply', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/application/v6/scopes/apply' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Application.V6.Scope.Apply(context.Background())
