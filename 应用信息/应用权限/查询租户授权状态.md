...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/application/v6/scopes");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/application/v6/scopes', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/application/v6/scopes' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Application.V6.Scope.List(context.Background())
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.application.v6.scope.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.application.v6.scope.list({
},
