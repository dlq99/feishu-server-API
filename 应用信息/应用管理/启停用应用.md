...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| enable | boolean | 否 | 启用/停用应用 <br> **示例**: true  |
...
$request = new Request('PUT', 'https://open.feishu.cn/open-apis/application/v6/applications/cli_a4517c8461f8100a/management', $headers, $body);
...
```bash
curl -i -X PUT 'https://open.feishu.cn/open-apis/application/v6/applications/cli_a4517c8461f8100a/management' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.application.v6.application_management.update failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.application.v6.applicationManagement.update({
        path: {
                app_id:'cli_a4517c8461f8100a',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/application/v6/applications/cli_a4517c8461f8100a/management");
client.Timeout = -1;
