...
| user_ids | undefined[] | 是 | 用户 ID 列表（ID 类型与 user_id_type 的取值一致。如果是商店应用，因不支持获取用户 user ID 权限，所以无法使用 user_id 类型的用户 ID） <br> **示例**:  <br> **数据校验规则**:<br>最大长度: 50最小长度: 1 |
...
func main() {
	...
	// 创建请求对象
	req := larkim.NewBotTimeSentiveFeedCardReqBuilder().
		UserIdType(`open_id`).
		Body(larkim.NewBotTimeSentiveFeedCardReqBodyBuilder().
			TimeSensitive(true).
	...
	// 发起请求
	resp, err := client.Im.V2.FeedCard.BotTimeSentive(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v2.feed_card.bot_time_sentive failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.im.v2.feedCard.botTimeSentive({
        params: {
                user_id_type:'open_id',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v2/feed_cards/bot_time_sentive?user_id_type=open_id");
client.Timeout = -1;
...
$request = new Request('PATCH', 'https://open.feishu.cn/open-apis/im/v2/feed_cards/bot_time_sentive?user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X PATCH 'https://open.feishu.cn/open-apis/im/v2/feed_cards/bot_time_sentive?user_id_type=open_id' \
