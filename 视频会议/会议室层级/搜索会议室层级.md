...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/vc/v1/room_levels/search?custom_level_ids=1000%2C1001");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/vc/v1/room_levels/search?custom_level_ids=1000%2C1001', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/vc/v1/room_levels/search?custom_level_ids=1000%2C1001' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.vc.v1.room_level.search failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
