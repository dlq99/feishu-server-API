...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| timezone | integer | 否 | 录制文件时间显示使用的时区[-12,12] <br> **示例**: 8  |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.vc.v1.meeting_recording.start failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.vc.v1.meetingRecording.start({
        path: {
                meeting_id:'6911188411932033028',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/vc/v1/meetings/6911188411932033028/recording/start");
client.Timeout = -1;
...
$request = new Request('PATCH', 'https://open.feishu.cn/open-apis/vc/v1/meetings/6911188411932033028/recording/start', $headers, $body);
...
```bash
curl -i -X PATCH 'https://open.feishu.cn/open-apis/vc/v1/meetings/6911188411932033028/recording/start' \
