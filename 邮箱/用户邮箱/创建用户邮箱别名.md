...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/mail/v1/user_mailboxes/user@xxx.xx/aliases");
client.Timeout = -1;
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/mail/v1/user_mailboxes/user@xxx.xx/aliases', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/mail/v1/user_mailboxes/user@xxx.xx/aliases' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Mail.V1.UserMailboxAlias.Create(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.user_mailbox_alias.create failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.mail.v1.userMailboxAlias.create({
        path: {
                user_mailbox_id:'user@xxx.xx',
        },
        data: {
