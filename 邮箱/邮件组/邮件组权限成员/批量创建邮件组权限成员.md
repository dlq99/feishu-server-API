...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| items | mailgroup.permission_member[] | 否 | 本次添加的邮件组权限成员列表 <br> **示例**:  <br> **数据校验规则**:<br>最大长度: 200最小长度: 1 |

**类型额外说明**:

- **mailgroup.permission_member** 类型说明:

  基本属性:

   permission_member_id: string <br> user_id: string <br> department_id: string <br> email: string <br> type: string <br>
      ...
      BatchCreateMailgroupPermissionMemberReq req = BatchCreateMailgroupPermissionMemberReq.newBuilder()
             ...
             .batchCreateMailgroupPermissionMemberReqBody(BatchCreateMailgroupPermissionMemberReqBody.newBuilder()
                 .items(new MailgroupPermissionMember[]{
                    MailgroupPermissionMember.newBuilder()
                      .userId("xxxxxxxxxx")
                      .type("USER")
                      .build(),
                    MailgroupPermissionMember.newBuilder()
...
client.mail.v1.mailgroupPermissionMember.batchCreate({
        path: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/mail/v1/mailgroups/xxxxxxxxxxxxxxx%20or%20test_mail_group@xxx.xx/permission_members/batch_create?department_id_type=open_department_id&user_id_type=open_id");
client.Timeout = -1;
var request = new RestRequest(Method.POST);
request.AddHeader("Authorization", "Bearer t-7f1b******8e560");
request.AddHeader("Content-Type", "application/json");
var body = "{\"items\":[{\"type\":\"USER\",\"user_id\":\"xxxxxxxxxx\"},{\"department_id\":\"xxxxxxxxxx\",\"type\":\"DEPARTMENT\"},{\"email\":\"xxx@xx.x\",\"type\":\"MAIL_GROUP\"},{\"email\":\"xxx@xx.x\",\"type\":\"PUBLIC_MAILBOX\"}]}";
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/mail/v1/mailgroups/xxxxxxxxxxxxxxx%20or%20test_mail_group@xxx.xx/permission_members/batch_create?department_id_type=open_department_id&user_id_type=open_id', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/mail/v1/mailgroups/xxxxxxxxxxxxxxx%20or%20test_mail_group@xxx.xx/permission_members/batch_create?department_id_type=open_department_id&user_id_type=open_id' \
...
func main() {
	...
	req := larkmail.NewBatchCreateMailgroupPermissionMemberReqBuilder().
		...
		Body(larkmail.NewBatchCreateMailgroupPermissionMemberReqBodyBuilder().
			Items([]*larkmail.MailgroupPermissionMember{
				larkmail.NewMailgroupPermissionMemberBuilder().
					UserId(`xxxxxxxxxx`).
					Type(`USER`).
					Build(),
				larkmail.NewMailgroupPermissionMemberBuilder().
...
def main():
    ...
    request: BatchCreateMailgroupPermissionMemberRequest = BatchCreateMailgroupPermissionMemberRequest.builder() \
        ...
        .request_body(BatchCreateMailgroupPermissionMemberRequestBody.builder()
            .items([MailgroupPermissionMember.builder()
                .user_id("xxxxxxxxxx")
                .type("USER")
                .build(), 
                MailgroupPermissionMember.builder()
                .department_id("xxxxxxxxxx")
                .type("DEPARTMENT")
                .build(), 
                MailgroupPermissionMember.builder()
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.mailgroup_permission_member.batch_create failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
