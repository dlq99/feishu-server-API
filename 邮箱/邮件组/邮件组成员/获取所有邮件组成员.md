...
| 支持的应用类型 | custom |
| 权限要求 | 获取用户 user ID <br> 查询、创建、修改、删除邮件组 <br> 查询邮件组 |
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/mail/v1/mailgroups/xxxxxxxxxxxxxxx%20or%20test_mail_group@xxx.xx/members?department_id_type=open_department_id&page_size=10&page_token=xxx&user_id_type=open_id' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.mailgroup_member.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.mail.v1.mailgroupMember.list({
        path: {
                mailgroup_id:'xxxxxxxxxxxxxxx or test_mail_group@xxx.xx',
        },
        params: {
...
(async () => {
    for await (const item of await client.mail.v1.mailgroupMember.listWithIterator({
            path: {
                    mailgroup_id:'xxxxxxxxxxxxxxx or test_mail_group@xxx.xx',
            },
            params: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/mail/v1/mailgroups/xxxxxxxxxxxxxxx%20or%20test_mail_group@xxx.xx/members?department_id_type=open_department_id&page_size=10&page_token=xxx&user_id_type=open_id");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/mail/v1/mailgroups/xxxxxxxxxxxxxxx%20or%20test_mail_group@xxx.xx/members?department_id_type=open_department_id&page_size=10&page_token=xxx&user_id_type=open_id', $headers, $body);
