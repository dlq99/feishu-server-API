...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.public_mailbox_member.clear failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.mail.v1.publicMailboxMember.clear({
        path: {
                public_mailbox_id:'xxxxxxxxxxxxxxx 或 test_public_mailbox@xxx.xx',
        },
},
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxxxxxxxxxxx%20%E6%88%96%20test_public_mailbox@xxx.xx/members/clear");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('POST', 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxxxxxxxxxxx%20%E6%88%96%20test_public_mailbox@xxx.xx/members/clear', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxxxxxxxxxxx%20%E6%88%96%20test_public_mailbox@xxx.xx/members/clear' \
