...
func main() {
	...
	// 发起请求
	resp, err := client.Mail.V1.PublicMailboxAlias.Delete(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.public_mailbox_alias.delete failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.mail.v1.publicMailboxAlias.delete({
        path: {
                public_mailbox_id:'xxxxxx 或 xxx@xx.xxx',
                alias_id:'xxx@xx.xxx',
        },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxx%20%E6%88%96%20xxx@xx.xxx/aliases/xxx@xx.xxx");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('DELETE', 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxx%20%E6%88%96%20xxx@xx.xxx/aliases/xxx@xx.xxx', $headers, $body);
...
```bash
curl -i -X DELETE 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxx%20%E6%88%96%20xxx@xx.xxx/aliases/xxx@xx.xxx' \
