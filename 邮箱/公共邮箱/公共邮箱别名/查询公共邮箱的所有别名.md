...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxx%20%E6%88%96%20xxx@xx.xxx/aliases', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxx%20%E6%88%96%20xxx@xx.xxx/aliases' \
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.public_mailbox_alias.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.mail.v1.publicMailboxAlias.list({
        path: {
                public_mailbox_id:'xxxxxx 或 xxx@xx.xxx',
        },
},
