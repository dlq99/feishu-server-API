...
| 支持的应用类型 | custom |
| 权限要求 | 查询公共邮箱 <br> 查看公共邮箱数据驻留地 <br> 查询、创建、修改公共邮箱 |
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxxxxxxxxxxx%20%E6%88%96%20test_public_mailbox@xxx.xx', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/mail/v1/public_mailboxes/xxxxxxxxxxxxxxx%20%E6%88%96%20test_public_mailbox@xxx.xx' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Mail.V1.PublicMailbox.Get(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.mail.v1.public_mailbox.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.mail.v1.publicMailbox.get({
        path: {
                public_mailbox_id:'xxxxxxxxxxxxxxx 或 test_public_mailbox@xxx.xx',
        },
},
