...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| tab_ids | undefined[] | 否 | 会话标签页 ID 列表，ID 可以在[添加会话标签页](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/create)或者[拉取会话标签页](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/list_tabs)接口返回值中获取。<br><br>**注意**：<br><br>- 必须包含会话内全部标签页的 ID。<br>- 当前参数的排序，对应会话内从左往右的排序。<br>- 会话内消息类型的标签页固定在第一顺位。 <br> **示例**: ["7101214603622940671", "7101214603622940672"]  |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v1.chat_tab.sort_tabs failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class SortTabsChatTabSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 发起请求
      SortTabsChatTabResp resp = client.im().v1().chatTab().sortTabs(req);
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v1/chats/oc_a0553eda9014c201e6969b478895c230/chat_tabs/sort_tabs");
client.Timeout = -1;
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/im/v1/chats/oc_a0553eda9014c201e6969b478895c230/chat_tabs/sort_tabs', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/im/v1/chats/oc_a0553eda9014c201e6969b478895c230/chat_tabs/sort_tabs' \
