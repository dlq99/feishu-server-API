...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| chat_menu_top_level_ids | undefined[] | 是 | 通过一级菜单的 ID 进行排序。数组内的元素排序对应群组内一级菜单从左往右的排序。ID 可通过 [获取群菜单](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-menu_tree/get) 接口获取。<br><br>**说明**：进行排序的 ID 列表需要跟群内存在的一级菜单 ID 列表对齐。 <br> **示例**: 7156553273518882844  |
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.im.v1.chat_menu_tree.sort failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
client.im.v1.chatMenuTree.sort({
        path: {
                chat_id:'oc_a0553eda9014c201e6969b478895c230',
        },
        data: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/im/v1/chats/oc_a0553eda9014c201e6969b478895c230/menu_tree/sort");
client.Timeout = -1;
...
$request = new Request('POST', 'https://open.feishu.cn/open-apis/im/v1/chats/oc_a0553eda9014c201e6969b478895c230/menu_tree/sort', $headers, $body);
...
```bash
curl -i -X POST 'https://open.feishu.cn/open-apis/im/v1/chats/oc_a0553eda9014c201e6969b478895c230/menu_tree/sort' \
...
func main() {
	...
	// 发起请求
	resp, err := client.Im.V1.ChatMenuTree.Sort(context.Background(), req)
