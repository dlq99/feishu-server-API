...
| 支持的应用类型 | custom  isv |
| 权限要求 | 更新招聘员工信息 <br> 获取用户 user ID <br> 获取招聘员工信息 |
...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| employee_id | string | 员工ID，可通过[通过投递 ID 获取入职信息](/ssl:ttdoc/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/employee/get_by_application)接口获取 |
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/hire/v1/employees/7379910335417927975?department_id_type=%22department_id%22&employee_type_id_type=%22people_admin_employee_type_id%22&job_family_id_type=%22people_admin_job_category_id%22&job_level_id_type=%22people_admin_job_level_id%22&user_id_type=open_id' \
...
func main() {
	...
	req := larkhire.NewGetEmployeeReqBuilder().
		EmployeeId(`7379910335417927975`).
		UserIdType(`open_id`).
	...
	// 发起请求
	resp, err := client.Hire.V1.Employee.Get(context.Background(), req)
...
def main():
    ...
    request: GetEmployeeRequest = GetEmployeeRequest.builder() \
        .employee_id("7379910335417927975") \
    ...
    if not response.success():
        lark.logger.error(
            f"client.hire.v1.employee.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/hire/v1/employees/7379910335417927975?department_id_type=%22department_id%22&employee_type_id_type=%22people_admin_employee_type_id%22&job_family_id_type=%22people_admin_job_category_id%22&job_level_id_type=%22people_admin_job_level_id%22&user_id_type=open_id");
client.Timeout = -1;
