...
| 参数名 | 类型 |  描述 |
| ------ | ---- | ---- |
| interview_record_id | string | 面试评价 ID，可通过[获取面试信息](/ssl:ttdoc/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/interview/list)接口获取 |
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/hire/v2/interview_records/7171693733661327361?user_id_type=open_id' \
...
func main() {
	...
	// 创建请求对象
	req := larkhire.NewGetInterviewRecordReqBuilder().
		InterviewRecordId(`7171693733661327361`).
		UserIdType(`open_id`).
		Build()

	// 发起请求
	resp, err := client.Hire.V2.InterviewRecord.Get(context.Background(), req)
...
def main():
    ...
    request: GetInterviewRecordRequest = GetInterviewRecordRequest.builder() \
        .interview_record_id("7171693733661327361") \
        .user_id_type("open_id") \
        .build()

    # 发起请求
    response: GetInterviewRecordResponse = client.hire.v2.interview_record.get(request)

    # 处理失败返回
    if not response.success():
        lark.logger.error(
            f"client.hire.v2.interview_record.get failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
...
public class GetInterviewRecordSample{
  ...
  public static void main(String arg[]) throws Exception {
      ...
      // 发起请求
      GetInterviewRecordResp resp = client.hire().v2().interviewRecord().get(req);
...
client.hire.v2.interviewRecord.get({
        path: {
                interview_record_id:'7171693733661327361',
        },
        params: {
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/hire/v2/interview_records/7171693733661327361?user_id_type=open_id");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/hire/v2/interview_records/7171693733661327361?user_id_type=open_id', $headers, $body);
