...
| 参数名 | 类型 | 必填 | 描述 |
| ------ | ---- | ---- | ---- |
| interview_feedback_form_ids | array | 否 | 面试评价表 ID 列表，使用此参数时不再分页 |
| page_size | integer | 否 | 分页大小。最大值为100，默认值为0 |
...
client.hire.v1.interviewFeedbackForm.list({
        params: {
                page_size:10,
                page_token:'eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0=',
        },
...
(async () => {
    for await (const item of await client.hire.v1.interviewFeedbackForm.listWithIterator({
            params: {
                        page_size:10,
            },
    },
...
```csharp
var client = new RestClient("https://open.feishu.cn/open-apis/hire/v1/interview_feedback_forms?page_size=10&page_token=eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0%3D");
client.Timeout = -1;
...
$body = 'null';
$request = new Request('GET', 'https://open.feishu.cn/open-apis/hire/v1/interview_feedback_forms?page_size=10&page_token=eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0%3D', $headers, $body);
...
```bash
curl -i -X GET 'https://open.feishu.cn/open-apis/hire/v1/interview_feedback_forms?page_size=10&page_token=eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0%3D' \
...
func main() {
	...
	// 创建请求对象
	req := larkhire.NewListInterviewFeedbackFormReqBuilder().
		PageSize(10).
		PageToken(`eVQrYzJBNDNONlk4VFZBZVlSdzlKdFJ4bVVHVExENDNKVHoxaVdiVnViQT0=`).
		Build()

	// 发起请求
	resp, err := client.Hire.V1.InterviewFeedbackForm.List(context.Background(), req)
...
def main():
    ...
    if not response.success():
        lark.logger.error(
            f"client.hire.v1.interview_feedback_form.list failed, code: {response.code}, msg: {response.msg}, log_id: {response.get_log_id()}, resp: \n{json.dumps(json.loads(response.raw.content), indent=4, ensure_ascii=False)}")
        return
